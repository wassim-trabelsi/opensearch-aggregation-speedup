...
3. Should users prefer aggregation-based queries for better performance, or is this a bug that should be fixed?

## Impact

This performance difference forces users to use workarounds (aggregations) instead of the more straightforward `size` parameter, which:

- Makes queries more complex
- Requires understanding aggregation syntax
- May not be intuitive for users expecting direct hit returns to be faster
